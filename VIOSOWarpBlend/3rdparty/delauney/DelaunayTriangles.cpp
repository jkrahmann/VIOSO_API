...
#ifdef WIN64
	#define TRIUNSIGNEDLONG unsigned __int64
#else
...
typedef DELAUNAY_REAL **triangle;            /* Really:  typedef triangle *triangle   */
...
typedef DELAUNAY_REAL **subseg;                  /* Really:  typedef subseg *subseg   */
...
struct badtriang {
  ...
  DELAUNAY_REAL key;                             /* cos^2 of smallest (apical) angle. */
...
struct event {
  DELAUNAY_REAL xkey, ykey;                              /* Coordinates of the event. */
...
DELAUNAY_REAL splitter;       /* Used to split DELAUNAY_REAL factors for exact multiplication. */
DELAUNAY_REAL epsilon;                             /* Floating-point machine epsilon. */
DELAUNAY_REAL resulterrbound;
DELAUNAY_REAL ccwerrboundA, ccwerrboundB, ccwerrboundC;
DELAUNAY_REAL iccerrboundA, iccerrboundB, iccerrboundC;
DELAUNAY_REAL o3derrboundA, o3derrboundB, o3derrboundC;
...
/* Other variables. */
  ...
  DELAUNAY_REAL xmin, xmax, ymin, ymax;                            /* x and y bounds. */
  DELAUNAY_REAL xminextreme;      /* Nonexistent x value used as a flag in sweepline. */
...
/* Read the instructions to find out the meaning of these switches.          */
  ...
  int nobisect;
  int steiner;
  DELAUNAY_REAL minangle, goodangle, offconstant;
  DELAUNAY_REAL maxarea;
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((TRIUNSIGNEDLONG) (ptr) & (TRIUNSIGNEDLONG) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((TRIUNSIGNEDLONG) (ptr) ^ (TRIUNSIGNEDLONG) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((TRIUNSIGNEDLONG) (otri).tri | (TRIUNSIGNEDLONG) (otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((TRIUNSIGNEDLONG) (otri).tri[6] | (TRIUNSIGNEDLONG) 2l)
...
#define infected(otri)                                                        \
  (((TRIUNSIGNEDLONG) (otri).tri[6] & (TRIUNSIGNEDLONG) 2l) != 0l)

/* Check or set a triangle's attributes.                                     */

#define elemattribute(otri, attnum)                                           \
  ((DELAUNAY_REAL *) (otri).tri)[m->elemattribindex + (attnum)]

#define setelemattribute(otri, attnum, value)                                 \
  ((DELAUNAY_REAL *) (otri).tri)[m->elemattribindex + (attnum)] = value

/* Check or set a triangle's maximum area bound.                             */

#define areabound(otri)  ((DELAUNAY_REAL *) (otri).tri)[m->areaboundindex]

#define setareabound(otri, value)                                             \
  ((DELAUNAY_REAL *) (otri).tri)[m->areaboundindex] = value
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((TRIUNSIGNEDLONG) (sptr) & (TRIUNSIGNEDLONG) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((TRIUNSIGNEDLONG) (sptr) & ~ (TRIUNSIGNEDLONG) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((TRIUNSIGNEDLONG) (osub).ss | (TRIUNSIGNEDLONG) (osub).ssorient)
...
int triunsuitable(vertex triorg, vertex tridest, vertex triapex, DELAUNAY_REAL area)
...
DELAUNAY_REAL area;                                      /* The area of the triangle. */
#endif /* not ANSI_DECLARATORS */

{
  DELAUNAY_REAL dxoa, dxda, dxod;
  DELAUNAY_REAL dyoa, dyda, dyod;
  DELAUNAY_REAL oalen, dalen, odlen;
  DELAUNAY_REAL maxlen;
...

{
  ...
  //memptr = (VOID *) new BYTE[(unsigned int) size];
  if (memptr == (VOID *) NULL) {
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'q') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            b->minangle = (DELAUNAY_REAL) strtod(workstring, (char **) NULL);
	  } else {
            b->minangle = 20.0;
	  }
	}
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            b->maxarea = (DELAUNAY_REAL) strtod(workstring, (char **) NULL);
            if (b->maxarea <= 0.0) {
...

{
  ...
  printf("triangle x%lx with orientation %d:\n", (unsigned long)(uintptr_t)(t->tri), t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == m->dummytri) {
    printf("    [0] = Outer space\n");
  } else {
    printf("    [0] = x%lx  %d\n", (unsigned long)(uintptr_t)(printtri.tri), printtri.orient);
  }
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (unsigned long)(uintptr_t)(printvertex), printvertex[0], printvertex[1]);
  dest(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Dest  [%d] = NULL\n", (t->orient + 2) % 3 + 3);
  else
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (unsigned long)(uintptr_t)(printvertex), printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
  else
    printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
           t->orient + 3, (unsigned long)(uintptr_t)(printvertex), printvertex[0], printvertex[1]);

  if (b->usesegments) {
    sdecode(t->tri[6], printsh);
    if (printsh.ss != m->dummysub) {
      printf("    [6] = x%lx  %d\n", (unsigned long)(uintptr_t)(printsh.ss), printsh.ssorient);
    }
...

{
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           2 + s->ssorient, (unsigned long)(uintptr_t)(printvertex), printvertex[0], printvertex[1]);
  sdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
  else
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           3 - s->ssorient, (unsigned long)(uintptr_t)(printvertex), printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (unsigned long)(uintptr_t)(printvertex), printvertex[0], printvertex[1]);
  segdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Segment dest  [%d] = NULL\n", 5 - s->ssorient);
  else
    printf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
           5 - s->ssorient, (unsigned long)(uintptr_t)(printvertex), printvertex[0], printvertex[1]);
}
...

{
  ...
  alignptr = (TRIUNSIGNEDLONG) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
    (alignptr + (TRIUNSIGNEDLONG) pool->alignbytes -
     (alignptr % (TRIUNSIGNEDLONG) pool->alignbytes));
...

{
  ...
  alignptr = (TRIUNSIGNEDLONG) (pool->pathblock + 1);
...

{
  ...
  alignptr = (TRIUNSIGNEDLONG) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (TRIUNSIGNEDLONG) m->triangles.alignbytes -
     (alignptr % (TRIUNSIGNEDLONG) m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    alignptr = (TRIUNSIGNEDLONG) m->dummysubbase;
    m->dummysub = (subseg *)
      (alignptr + (TRIUNSIGNEDLONG) m->subsegs.alignbytes -
       (alignptr % (TRIUNSIGNEDLONG) m->subsegs.alignbytes));
...

{
  ...
  m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(DELAUNAY_REAL) +
                        sizeof(int) - 1) /
                       sizeof(int);
...

{
  ...
  m->elemattribindex = (trisize + sizeof(DELAUNAY_REAL) - 1) / sizeof(DELAUNAY_REAL);
  ...
  if (b->vararea) {
    trisize = (m->areaboundindex + 1) * sizeof(DELAUNAY_REAL);
  } else if (m->eextras + b->regionattrib > 0) {
    trisize = m->areaboundindex * sizeof(DELAUNAY_REAL);
  }
...

{
  ...
  alignptr = (TRIUNSIGNEDLONG) (getblock + 1);
  foundvertex = (char *) (alignptr + (TRIUNSIGNEDLONG) m->vertices.alignbytes -
                          (alignptr % (TRIUNSIGNEDLONG) m->vertices.alignbytes));
...
#define Split(a, ahi, alo) \
  c = (DELAUNAY_REAL) (splitter * a); \
  abig = (DELAUNAY_REAL) (c - a); \
  ahi = c - abig; \
  alo = a - ahi
...
#ifdef ANSI_DECLARATORS
int fast_expansion_sum_zeroelim(int elen, DELAUNAY_REAL *e, int flen, DELAUNAY_REAL *f, DELAUNAY_REAL *h)
#else /* not ANSI_DECLARATORS */
...
{
  DELAUNAY_REAL Q;
  INEXACT DELAUNAY_REAL Qnew;
  INEXACT DELAUNAY_REAL hh;
  INEXACT DELAUNAY_REAL bvirt;
  DELAUNAY_REAL avirt, bround, around;
  int eindex, findex, hindex;
  DELAUNAY_REAL enow, fnow;
...
#ifdef ANSI_DECLARATORS
int scale_expansion_zeroelim(int elen, DELAUNAY_REAL *e, DELAUNAY_REAL b, DELAUNAY_REAL *h)
#else /* not ANSI_DECLARATORS */
...
{
  INEXACT DELAUNAY_REAL Q, sum;
  DELAUNAY_REAL hh;
  INEXACT DELAUNAY_REAL product1;
  DELAUNAY_REAL product0;
  int eindex, hindex;
  DELAUNAY_REAL enow;
  INEXACT DELAUNAY_REAL bvirt;
  DELAUNAY_REAL avirt, bround, around;
  INEXACT DELAUNAY_REAL c;
  INEXACT DELAUNAY_REAL abig;
  DELAUNAY_REAL ahi, alo, bhi, blo;
  DELAUNAY_REAL err1, err2, err3;
...
#ifdef ANSI_DECLARATORS
DELAUNAY_REAL estimate(int elen, DELAUNAY_REAL *e)
#else /* not ANSI_DECLARATORS */
DELAUNAY_REAL estimate(elen, e)
int elen;
DELAUNAY_REAL *e;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
DELAUNAY_REAL counterclockwiseadapt(vertex pa, vertex pb, vertex pc, DELAUNAY_REAL detsum)
#else /* not ANSI_DECLARATORS */
DELAUNAY_REAL counterclockwiseadapt(pa, pb, pc, detsum)
vertex pa;
vertex pb;
vertex pc;
DELAUNAY_REAL detsum;
#endif /* not ANSI_DECLARATORS */

{
  INEXACT DELAUNAY_REAL acx, acy, bcx, bcy;
  DELAUNAY_REAL acxtail, acytail, bcxtail, bcytail;
  INEXACT DELAUNAY_REAL detleft, detright;
  DELAUNAY_REAL detlefttail, detrighttail;
  DELAUNAY_REAL det, errbound;
  DELAUNAY_REAL B[4], C1[8], C2[12], D[16];
  INEXACT DELAUNAY_REAL B3;
  int C1length, C2length, Dlength;
  DELAUNAY_REAL u[4];
  INEXACT DELAUNAY_REAL u3;
  INEXACT DELAUNAY_REAL s1, t1;
  DELAUNAY_REAL s0, t0;
  ...
  DELAUNAY_REAL ahi, alo, bhi, blo;
  DELAUNAY_REAL err1, err2, err3;
  INEXACT DELAUNAY_REAL _i, _j;
  DELAUNAY_REAL _0;

  acx = (DELAUNAY_REAL) (pa[0] - pc[0]);
  bcx = (DELAUNAY_REAL) (pb[0] - pc[0]);
  acy = (DELAUNAY_REAL) (pa[1] - pc[1]);
  bcy = (DELAUNAY_REAL) (pb[1] - pc[1]);
...
#ifdef ANSI_DECLARATORS
DELAUNAY_REAL counterclockwise(struct mesh *m, struct behavior *b,
                      vertex pa, vertex pb, vertex pc)
#else /* not ANSI_DECLARATORS */
DELAUNAY_REAL counterclockwise(m, b, pa, pb, pc)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  DELAUNAY_REAL detleft, detright, det;
  DELAUNAY_REAL detsum, errbound;
...
#ifdef ANSI_DECLARATORS
DELAUNAY_REAL incircleadapt(vertex pa, vertex pb, vertex pc, vertex pd, DELAUNAY_REAL permanent)
#else /* not ANSI_DECLARATORS */
DELAUNAY_REAL incircleadapt(pa, pb, pc, pd, permanent)
vertex pa;
vertex pb;
vertex pc;
vertex pd;
DELAUNAY_REAL permanent;
#endif /* not ANSI_DECLARATORS */

{
  INEXACT DELAUNAY_REAL adx, bdx, cdx, ady, bdy, cdy;
  DELAUNAY_REAL det, errbound;

  INEXACT DELAUNAY_REAL bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
  DELAUNAY_REAL bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
  DELAUNAY_REAL bc[4], ca[4], ab[4];
  INEXACT DELAUNAY_REAL bc3, ca3, ab3;
  DELAUNAY_REAL axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
  int axbclen, axxbclen, aybclen, ayybclen, alen;
  DELAUNAY_REAL bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
  int bxcalen, bxxcalen, bycalen, byycalen, blen;
  DELAUNAY_REAL cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
  int cxablen, cxxablen, cyablen, cyyablen, clen;
  DELAUNAY_REAL abdet[64];
  int ablen;
  DELAUNAY_REAL fin1[1152], fin2[1152];
  DELAUNAY_REAL *finnow, *finother, *finswap;
  int finlength;

  DELAUNAY_REAL adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
  INEXACT DELAUNAY_REAL adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
  DELAUNAY_REAL adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
  DELAUNAY_REAL aa[4], bb[4], cc[4];
  INEXACT DELAUNAY_REAL aa3, bb3, cc3;
  INEXACT DELAUNAY_REAL ti1, tj1;
  DELAUNAY_REAL ti0, tj0;
  DELAUNAY_REAL u[4], v[4];
  INEXACT DELAUNAY_REAL u3, v3;
  DELAUNAY_REAL temp8[8], temp16a[16], temp16b[16], temp16c[16];
  DELAUNAY_REAL temp32a[32], temp32b[32], temp48[48], temp64[64];
  int temp8len, temp16alen, temp16blen, temp16clen;
  int temp32alen, temp32blen, temp48len, temp64len;
  DELAUNAY_REAL axtbb[8], axtcc[8], aytbb[8], aytcc[8];
  int axtbblen, axtcclen, aytbblen, aytcclen;
  DELAUNAY_REAL bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
  int bxtaalen, bxtcclen, bytaalen, bytcclen;
  DELAUNAY_REAL cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
  int cxtaalen, cxtbblen, cytaalen, cytbblen;
  DELAUNAY_REAL axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
  int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
  DELAUNAY_REAL axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
  int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  DELAUNAY_REAL axtbctt[8], aytbctt[8], bxtcatt[8];
  DELAUNAY_REAL bytcatt[8], cxtabtt[8], cytabtt[8];
  int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
  DELAUNAY_REAL abt[8], bct[8], cat[8];
  int abtlen, bctlen, catlen;
  DELAUNAY_REAL abtt[4], bctt[4], catt[4];
  int abttlen, bcttlen, cattlen;
  INEXACT DELAUNAY_REAL abtt3, bctt3, catt3;
  DELAUNAY_REAL negate;
  ...
  adx = (DELAUNAY_REAL) (pa[0] - pd[0]);
  bdx = (DELAUNAY_REAL) (pb[0] - pd[0]);
  cdx = (DELAUNAY_REAL) (pc[0] - pd[0]);
  ady = (DELAUNAY_REAL) (pa[1] - pd[1]);
  bdy = (DELAUNAY_REAL) (pb[1] - pd[1]);
  cdy = (DELAUNAY_REAL) (pc[1] - pd[1]);
...
#ifdef ANSI_DECLARATORS
DELAUNAY_REAL incircle(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
DELAUNAY_REAL incircle(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  DELAUNAY_REAL adx, bdx, cdx, ady, bdy, cdy;
  DELAUNAY_REAL bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  DELAUNAY_REAL alift, blift, clift;
  DELAUNAY_REAL det;
  DELAUNAY_REAL permanent, errbound;
...
#ifdef ANSI_DECLARATORS
DELAUNAY_REAL orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
                   DELAUNAY_REAL aheight, DELAUNAY_REAL bheight, DELAUNAY_REAL cheight, DELAUNAY_REAL dheight,
                   DELAUNAY_REAL permanent)
#else /* not ANSI_DECLARATORS */
DELAUNAY_REAL orient3dadapt(pa, pb, pc, pd,
                   aheight, bheight, cheight, dheight, permanent)
vertex pa;
vertex pb;
vertex pc;
vertex pd;
DELAUNAY_REAL aheight;
DELAUNAY_REAL bheight;
DELAUNAY_REAL cheight;
DELAUNAY_REAL dheight;
DELAUNAY_REAL permanent;
#endif /* not ANSI_DECLARATORS */

{
  INEXACT DELAUNAY_REAL adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  DELAUNAY_REAL det, errbound;
  ...
  DELAUNAY_REAL bc[4], ca[4], ab[4];
  INEXACT DELAUNAY_REAL bc3, ca3, ab3;
  DELAUNAY_REAL adet[8], bdet[8], cdet[8];
  int alen, blen, clen;
  DELAUNAY_REAL abdet[16];
  int ablen;
  DELAUNAY_REAL *finnow, *finother, *finswap;
  DELAUNAY_REAL fin1[192], fin2[192];
  int finlength;

  DELAUNAY_REAL adxtail, bdxtail, cdxtail;
  DELAUNAY_REAL adytail, bdytail, cdytail;
  DELAUNAY_REAL adheighttail, bdheighttail, cdheighttail;
  INEXACT DELAUNAY_REAL at_blarge, at_clarge;
  INEXACT DELAUNAY_REAL bt_clarge, bt_alarge;
  INEXACT DELAUNAY_REAL ct_alarge, ct_blarge;
  DELAUNAY_REAL at_b[4], at_c[4], bt_c[4], bt_a[4], ct_a[4], ct_b[4];
  int at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
  INEXACT DELAUNAY_REAL bdxt_cdy1, cdxt_bdy1, cdxt_ady1;
  INEXACT DELAUNAY_REAL adxt_cdy1, adxt_bdy1, bdxt_ady1;
  DELAUNAY_REAL bdxt_cdy0, cdxt_bdy0, cdxt_ady0;
  DELAUNAY_REAL adxt_cdy0, adxt_bdy0, bdxt_ady0;
  INEXACT DELAUNAY_REAL bdyt_cdx1, cdyt_bdx1, cdyt_adx1;
  INEXACT DELAUNAY_REAL adyt_cdx1, adyt_bdx1, bdyt_adx1;
  DELAUNAY_REAL bdyt_cdx0, cdyt_bdx0, cdyt_adx0;
  DELAUNAY_REAL adyt_cdx0, adyt_bdx0, bdyt_adx0;
  DELAUNAY_REAL bct[8], cat[8], abt[8];
  int bctlen, catlen, abtlen;
  INEXACT DELAUNAY_REAL bdxt_cdyt1, cdxt_bdyt1, cdxt_adyt1;
  INEXACT DELAUNAY_REAL adxt_cdyt1, adxt_bdyt1, bdxt_adyt1;
  DELAUNAY_REAL bdxt_cdyt0, cdxt_bdyt0, cdxt_adyt0;
  DELAUNAY_REAL adxt_cdyt0, adxt_bdyt0, bdxt_adyt0;
  DELAUNAY_REAL u[4], v[12], w[16];
  INEXACT DELAUNAY_REAL u3;
  int vlength, wlength;
  DELAUNAY_REAL negate;
  ...
  DELAUNAY_REAL ahi, alo, bhi, blo;
  DELAUNAY_REAL err1, err2, err3;
  INEXACT DELAUNAY_REAL _i, _j, _k;
  DELAUNAY_REAL _0;
  ...
  bdy = (DELAUNAY_REAL) (pb[1] - pd[1]);
  cdy = (DELAUNAY_REAL) (pc[1] - pd[1]);
  adheight = (DELAUNAY_REAL) (aheight - dheight);
  bdheight = (DELAUNAY_REAL) (bheight - dheight);
  cdheight = (DELAUNAY_REAL) (cheight - dheight);
...
#ifdef ANSI_DECLARATORS
DELAUNAY_REAL orient3d(struct mesh *m, struct behavior *b,
              vertex pa, vertex pb, vertex pc, vertex pd,
              DELAUNAY_REAL aheight, DELAUNAY_REAL bheight, DELAUNAY_REAL cheight, DELAUNAY_REAL dheight)
#else /* not ANSI_DECLARATORS */
DELAUNAY_REAL orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
{
  DELAUNAY_REAL adx, bdx, cdx, ady, bdy, cdy, adheight, bdheight, cdheight;
  DELAUNAY_REAL bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  DELAUNAY_REAL det;
  DELAUNAY_REAL permanent, errbound;
...
#ifdef ANSI_DECLARATORS
DELAUNAY_REAL nonregular(struct mesh *m, struct behavior *b,
                vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
DELAUNAY_REAL nonregular(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
void findcircumcenter(struct mesh *m, struct behavior *b,
                      ...
                      vertex circumcenter, DELAUNAY_REAL *xi, DELAUNAY_REAL *eta, int offcenter)
#else /* not ANSI_DECLARATORS */
...
{
  DELAUNAY_REAL xdo, ydo, xao, yao;
  DELAUNAY_REAL dodist, aodist, dadist;
  DELAUNAY_REAL denominator;
  DELAUNAY_REAL dx, dy, dxoff, dyoff;
...
{
  DELAUNAY_REAL length, multiplier;
...
void enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri,
                   DELAUNAY_REAL minedge, vertex enqapex, vertex enqorg, vertex enqdest)
#else /* not ANSI_DECLARATORS */
void enqueuebadtri(m, b, enqtri, minedge, enqapex, enqorg, enqdest)
struct mesh *m;
struct behavior *b;
struct otri *enqtri;
DELAUNAY_REAL minedge;
vertex enqapex;
vertex enqorg;
vertex enqdest;
#endif /* not ANSI_DECLARATORS */
...

{
  ...
  vertex joinvertex;
  DELAUNAY_REAL dxod, dyod, dxda, dyda, dxao, dyao;
  DELAUNAY_REAL dxod2, dyod2, dxda2, dyda2, dxao2, dyao2;
  DELAUNAY_REAL apexlen, orglen, destlen, minedge;
  DELAUNAY_REAL angle;
  DELAUNAY_REAL area;
  DELAUNAY_REAL dist1, dist2;
  subseg sptr;                      /* Temporary variable used by tspivot(). */
...

{
  ...
  DELAUNAY_REAL orgorient, destorient;
  int moveleft;
  triangle ptr;                         /* Temporary variable used by sym(). */
  subseg sptr;                      /* Temporary variable used by tspivot(). */
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (TRIUNSIGNEDLONG) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (TRIUNSIGNEDLONG) m->triangles.alignbytes -
                         (alignptr %
                          (TRIUNSIGNEDLONG) m->triangles.alignbytes));
...
#ifdef ANSI_DECLARATORS
DELAUNAY_REAL circletop(struct mesh *m, vertex pa, vertex pb, vertex pc, DELAUNAY_REAL ccwabc)
#else /* not ANSI_DECLARATORS */
DELAUNAY_REAL circletop(m, pa, pb, pc, ccwabc)
struct mesh *m;
vertex pa;
vertex pb;
vertex pc;
DELAUNAY_REAL ccwabc;
#endif /* not ANSI_DECLARATORS */

{
  DELAUNAY_REAL xac, yac, xbc, ybc, xab, yab;
  DELAUNAY_REAL aclen2, bclen2, ablen2;
...
struct splaynode *circletopinsert(struct mesh *m, struct behavior *b,
                                  struct splaynode *splayroot,
                                  struct otri *newkey,
                                  vertex pa, vertex pb, vertex pc, DELAUNAY_REAL topy)
#else /* not ANSI_DECLARATORS */
...
{
  DELAUNAY_REAL ccwabc;
  DELAUNAY_REAL xac, yac, xbc, ybc;
  DELAUNAY_REAL aclen2, bclen2;
  DELAUNAY_REAL searchpoint[2];
...

{
  ...
  DELAUNAY_REAL lefttest, righttest;
  int heapsize;
...
int reconstruct(struct mesh *m, struct behavior *b, int *trianglelist,
                DELAUNAY_REAL *triangleattriblist, DELAUNAY_REAL *trianglearealist,
...
struct behavior *b;
int *trianglelist;
DELAUNAY_REAL *triangleattriblist;
DELAUNAY_REAL *trianglearealist;
int elements;
int corners;
int attribs;
int *segmentlist;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < m->eextras; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        setelemattribute(triangleloop, j,
                         (DELAUNAY_REAL) strtod(stringptr, &stringptr));
      }
#endif /* not TRILIBRARY */
    }
    ...
    if (b->vararea) {
      ...
      if (*stringptr == '\0') {
        ...
        area = (DELAUNAY_REAL) strtod(stringptr, &stringptr);
      }
#endif /* not TRILIBRARY */
...

{
  ...
  vertex startvertex;
  vertex leftvertex, rightvertex;
  DELAUNAY_REAL leftccw, rightccw;
...

{
  ...
  DELAUNAY_REAL ex, ey;
  DELAUNAY_REAL tx, ty;
  DELAUNAY_REAL etx, ety;
  DELAUNAY_REAL split, denom;
  int i;
  triangle ptr;                       /* Temporary variable used by onext(). */
...
void regionplague(struct mesh *m, struct behavior *b,
                  DELAUNAY_REAL attribute, DELAUNAY_REAL area)
#else /* not ANSI_DECLARATORS */
void regionplague(m, b, attribute, area)
struct mesh *m;
struct behavior *b;
DELAUNAY_REAL attribute;
DELAUNAY_REAL area;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void carveholes(struct mesh *m, struct behavior *b, DELAUNAY_REAL *holelist, int holes,
                DELAUNAY_REAL *regionlist, int regions)
#else /* not ANSI_DECLARATORS */
void carveholes(m, b, holelist, holes, regionlist, regions)
struct mesh *m;
struct behavior *b;
DELAUNAY_REAL *holelist;
int holes;
DELAUNAY_REAL *regionlist;
int regions;
#endif /* not ANSI_DECLARATORS */
...
void precisionerror()
{
  ...
  printf("  arithmetic (by removing \"#define DELAUNAY_PRECISION_SINGLE\" from the\n");
  printf("  source file or \"-DSINGLE\" from the makefile).\n");
#endif /* DELAUNAY_PRECISION_SINGLE */
}
...

{
  ...
  vertex newvertex;
  enum insertvertexresult success;
  DELAUNAY_REAL segmentlength, nearestpoweroftwo;
  DELAUNAY_REAL split;
  DELAUNAY_REAL multiplier, divisor;
...

{
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    x = (DELAUNAY_REAL) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      printf("Error:  Vertex %d has no y coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
    y = (DELAUNAY_REAL) strtod(stringptr, &stringptr);
    vertexloop[0] = x;
    vertexloop[1] = y;
    ...
    for (j = 2; j < 2 + m->nextras; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        vertexloop[j] = (DELAUNAY_REAL) strtod(stringptr, &stringptr);
      }
    }
    if (nodemarkers) {
...
#ifdef ANSI_DECLARATORS
void transfernodes(struct mesh *m, struct behavior *b, DELAUNAY_REAL *pointlist,
                   DELAUNAY_REAL *pointattriblist, int *pointmarkerlist,
...
struct mesh *m;
struct behavior *b;
DELAUNAY_REAL *pointlist;
DELAUNAY_REAL *pointattriblist;
int *pointmarkerlist;
int numberofpoints;
...
void readholes(struct mesh *m, struct behavior *b,
               FILE *polyfile, char *polyfilename, DELAUNAY_REAL **hlist, int *holes,
               DELAUNAY_REAL **rlist, int *regions)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  *holes = (int) strtol(stringptr, &stringptr, 0);
  if (*holes > 0) {
    holelist = (DELAUNAY_REAL *) trimalloc(2 * *holes * (int) sizeof(DELAUNAY_REAL));
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      stringptr = readline(inputline, polyfile, polyfilename);
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        holelist[i] = (DELAUNAY_REAL) strtod(stringptr, &stringptr);
      }
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        holelist[i + 1] = (DELAUNAY_REAL) strtod(stringptr, &stringptr);
      }
    }
  } else {
    *hlist = (DELAUNAY_REAL *) NULL;
  }
  ...
  if ((b->regionattrib || b->vararea) && !b->refine) {
    ...
    *regions = (int) strtol(stringptr, &stringptr, 0);
    if (*regions > 0) {
      regionlist = (DELAUNAY_REAL *) trimalloc(4 * *regions * (int) sizeof(DELAUNAY_REAL));
      *rlist = regionlist;
      index = 0;
      for (i = 0; i < *regions; i++) {
        stringptr = readline(inputline, polyfile, polyfilename);
        ...
        if (*stringptr == '\0') {
          ...
          regionlist[index++] = (DELAUNAY_REAL) strtod(stringptr, &stringptr);
        }
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
...
#ifdef ANSI_DECLARATORS
void writenodes(struct mesh *m, struct behavior *b, DELAUNAY_REAL **pointlist,
                DELAUNAY_REAL **pointattriblist, int **pointmarkerlist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*pointlist == (DELAUNAY_REAL *) NULL) {
    *pointlist = (DELAUNAY_REAL *) trimalloc((int) (outvertices * 2 * sizeof(DELAUNAY_REAL)));
  }
  /* Allocate memory for output vertex attributes if necessary. */
  if ((m->nextras > 0) && (*pointattriblist == (DELAUNAY_REAL *) NULL)) {
    *pointattriblist = (DELAUNAY_REAL *) trimalloc((int) (outvertices * m->nextras *
                                                 sizeof(DELAUNAY_REAL)));
  }
...
void writeelements(struct mesh *m, struct behavior *b,
                   int **trianglelist, DELAUNAY_REAL **triangleattriblist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if ((m->eextras > 0) && (*triangleattriblist == (DELAUNAY_REAL *) NULL)) {
    *triangleattriblist = (DELAUNAY_REAL *) trimalloc((int) (m->triangles.items *
                                                    m->eextras *
                                                    sizeof(DELAUNAY_REAL)));
  }
  tlist = *trianglelist;
...
void writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
               DELAUNAY_REAL *holelist, int holes, DELAUNAY_REAL *regionlist, int regions,
               int argc, char **argv)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void writevoronoi(struct mesh *m, struct behavior *b, DELAUNAY_REAL **vpointlist,
                  DELAUNAY_REAL **vpointattriblist, int **vpointmarkerlist,
                  int **vedgelist, int **vedgemarkerlist, DELAUNAY_REAL **vnormlist)
#else /* not ANSI_DECLARATORS */
void writevoronoi(m, b, vpointlist, vpointattriblist, vpointmarkerlist,
                  vedgelist, vedgemarkerlist, vnormlist)
struct mesh *m;
struct behavior *b;
DELAUNAY_REAL **vpointlist;
DELAUNAY_REAL **vpointattriblist;
int **vpointmarkerlist;
int **vedgelist;
int **vedgemarkerlist;
DELAUNAY_REAL **vnormlist;
#endif /* not ANSI_DECLARATORS */
...

{
  ...
  if (*vpointlist == (DELAUNAY_REAL *) NULL) {
    *vpointlist = (DELAUNAY_REAL *) trimalloc((int) (m->triangles.items * 2 *
                                            sizeof(DELAUNAY_REAL)));
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == (DELAUNAY_REAL *) NULL) {
    *vpointattriblist = (DELAUNAY_REAL *) trimalloc((int) (m->triangles.items *
                                                  m->nextras * sizeof(DELAUNAY_REAL)));
  }
  ...
  *vedgemarkerlist = (int *) NULL;
  ...
  if (*vnormlist == (DELAUNAY_REAL *) NULL) {
    *vnormlist = (DELAUNAY_REAL *) trimalloc((int) (m->edges * 2 * sizeof(DELAUNAY_REAL)));
  }
  elist = *vedgelist;
  normlist = *vnormlist;
  coordindex = 0;
...

{
  struct otri triangleloop;
  vertex p[3];
  DELAUNAY_REAL cossquaretable[8];
  DELAUNAY_REAL ratiotable[16];
  DELAUNAY_REAL dx[3], dy[3];
  DELAUNAY_REAL edgelength[3];
  DELAUNAY_REAL dotproduct;
  DELAUNAY_REAL cossquare;
  DELAUNAY_REAL triarea;
  DELAUNAY_REAL shortest, longest;
  DELAUNAY_REAL trilongest2;
  DELAUNAY_REAL smallestarea, biggestarea;
  DELAUNAY_REAL triminaltitude2;
  DELAUNAY_REAL minaltitude;
  DELAUNAY_REAL triaspect2;
  DELAUNAY_REAL worstaspect;
  DELAUNAY_REAL smallestangle, biggestangle;
  DELAUNAY_REAL radconst, degconst;
  int angletable[18];
  int aspecttable[16];
  int aspectindex;
  int tendegree;
  ...
  degconst = 180.0 / PI;
  for (i = 0; i < 8; i++) {
    cossquaretable[i] = cos(radconst * (DELAUNAY_REAL) (i + 1));
...

{
  struct mesh m;
  struct behavior b;
  DELAUNAY_REAL *holearray;                                        /* Array of holes. */
  DELAUNAY_REAL *regionarray;   /* Array of regional attributes and area constraints. */
#ifndef TRILIBRARY
  FILE *polyfile;
#endif /* not TRILIBRARY */
#ifndef NO_TIMER
  ...
  if (b.poly || b.convex) {
    ...
    if (b.nopolywritten || b.noiterationnum) {
      ...
      if (b.poly) {
        ...
        out->holelist = (DELAUNAY_REAL *) NULL;
        out->regionlist = (DELAUNAY_REAL *) NULL;
      }
#else /* not TRILIBRARY */
...
void info()
{
	...
	{
		if(cont.pointlist)				free(cont.pointlist);
		if(cont.pointattributelist)		free(cont.pointattributelist);
		if(cont.pointmarkerlist)		free(cont.pointmarkerlist);
		if(cont.trianglelist)			free(cont.trianglelist);
		if(cont.triangleattributelist)	free(cont.triangleattributelist);
		if(cont.trianglearealist)		free(cont.trianglearealist);
		if(cont.neighborlist)			free(cont.neighborlist);
		if(cont.segmentlist)			free(cont.segmentlist);
		if(cont.segmentmarkerlist)		free(cont.segmentmarkerlist);
		if(cont.holelist)				free(cont.holelist);
		if(cont.regionlist)				free(cont.regionlist);
		if(cont.edgelist)				free(cont.edgelist);
		if(cont.edgemarkerlist)			free(cont.edgemarkerlist);
		if(cont.normlist)				free(cont.normlist);
		memset(&cont,0,sizeof(triangulateio));
	}
